...
} cpu_stats[MAX_CPUS + 1];  // +1 for total CPU

struct {
    unsigned long long prev_rx_bytes;
    unsigned long long prev_tx_bytes;
    double rx_speed;  // bytes per second
    double tx_speed;  // bytes per second
...
double get_cpu_usage(int cpu_num) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (strncmp(line, cpu_label, strlen(cpu_label)) == 0) {
            ...
            sscanf(line + strlen(cpu_label), "%llu %llu %llu %llu %llu %llu %llu %llu",
            ...
            int idx = cpu_num + 1;  // +1 because cpu_stats[0] is for total CPU
