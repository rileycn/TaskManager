...
double memory_history[2][HISTORY_SIZE];
const struct {
    double r, g, b;
} memory_colors[] = {
    {0.2, 0.7, 0.2},
...
void draw_cpu_graph(GtkWidget *widget, cairo_t *cr, double width, double height) {
    ...
    for (int cpu = -1; cpu < num_cpus; cpu++) {
        ...
        for (int j = 59; j >= 0; j--) {
            ...
            double y = graph_height - bottom_margin - (cpu_usage_history[cpu + 1][idx] * (graph_height - top_margin - bottom_margin) / 100.0);
            if (first) {
...
void draw_memory_graph(GtkWidget *widget, cairo_t *cr, double width, double height) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int j = 59; j >= 0; j--) {
            int idx = (history_index - (59 - j) + HISTORY_SIZE) % HISTORY_SIZE;
            double x = left_margin + (j * (width - left_margin - 10) / 60.0);
            double y = graph_y_offset + graph_height - bottom_margin - (memory_history[i][idx] * (graph_height - top_margin - bottom_margin) / 100.0);
            if (first) {
                cairo_move_to(cr, x, y);
                first = FALSE;
            }
    ...
    for (int i = 0; i < 2; i++) {
        cairo_set_source_rgb(cr, memory_colors[i].r, memory_colors[i].g, memory_colors[i].b);
        char label[40];
        snprintf(label, sizeof(label), "%s (%.1f%%)", labels[i], memory_history[i][history_index > 0 ? history_index - 1 : HISTORY_SIZE - 1]);
...
void draw_network_graph(GtkWidget *widget, cairo_t *cr, double width, double height) {
    ...
    for (int i = 0; i <= 5; i++) {
        ...
        cairo_move_to(cr, 5, graph_y_offset + graph_height - bottom_margin - (i * (graph_height - top_margin - bottom_margin) / 5.0));
        cairo_show_text(cr, label);
    }
    for (int i = 0; i <= 60; i += 10) {
        char label[10];
    ...
    for (int i = 0; i <= 20; i += 4) {
        double y = graph_y_offset + graph_height - bottom_margin - (i * (graph_height - top_margin - bottom_margin) / 20.0);
        cairo_move_to(cr, left_margin, y);
        cairo_line_to(cr, width - 10, y);
    }
    for (int i = 0; i <= 60; i += 10) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int j = 59; j >= 0; j--) {
            ...
            double y = graph_y_offset + graph_height - bottom_margin - (scaled_value * (graph_height - top_margin - bottom_margin) / max_value);
            if (first) {
                cairo_move_to(cr, x, y);
                first = FALSE;
            }
    ...
    for (int i = 0; i < 2; i++) {
        cairo_set_source_rgb(cr, network_colors[i].r, network_colors[i].g, network_colors[i].b);
        char label[40];
        snprintf(label, sizeof(label), "%s (%.1f KiB/s)", labels[i], network_history[i][history_index > 0 ? history_index - 1 : HISTORY_SIZE - 1]);
        cairo_move_to(cr, legend_x + 10, legend_y + i * 15);
        cairo_show_text(cr, label);
    }
}
